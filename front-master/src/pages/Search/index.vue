              ...
              <li class="yui3-u-1-5" v-for="(good,index) in goodsList" :key="good.id">
                <div class="list-wrap">
                  ...
                  <div class="operate">
                    ...
                    <a href="javaript:void(0);" class="sui-btn btn-bordered">收藏</a>
                  </div>
                </div>
              </li>
...
<script>
  ...
  export default {
    ...
    beforeMount(){
      // console.log(this.$route.query,'ooop');
    ...
    computed:{
      //mapGetters里面的写法，：传递的数值，因为getters计算是没有划分模块【home,search】
    ...
    methods:{
      ...
      //删除分类的名字
      removeCategoryName(){
        //把带给服务器的参数置空，还需要想服务器发请求
        //带给服务器的参数说明为{可有可无}时,如果属性值为空的字符串还是会把相应的字段带给服务器
        ...
        this.getData();
        //地址栏也需要修改：进行路由跳转
        //严谨：本意是删除query,如果路径当中有params参数不应该删除，鲁豫跳转应该带着
      ...
      changeOrder(flag){
        ...
        console.log(orginFlag,'0',orginSort,'1');
        //准备一个新的order属性值
        let newOrder = '';
        //判断的是多次点击的是不是同一个按钮
        if(flag == orginFlag) {
    ...
    watch:{
      ...
      $route(newValue,oldValue) {
        //每一次请求完毕，应该把相应的1、2、3级分类的id置空，让他接受下一次相应1、2、3id
        //分类名字与关键字不用清理，因为每次路由发生变化的时候，都会给他赋予新的数据
        this.searchParams.category1Id = undefined;
        this.searchParams.category2Id = undefined;
        this.searchParams.category3Id = undefined;
        //再次发请求之前整理服务器参数
